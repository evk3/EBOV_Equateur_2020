                        ...
                        if isinstance(k,bt.leaf): ## only interested in tips
                            for s in subgroups.keys():
                                if s in k.name: ## if name of tip satisfies condition
                                    ...
                                    print(k.name)
                                    print(k.numName)
                                    subgroups[s].append(k.numName) ## add the tip's numName to subrate group list - tips will be used to ID the common ancestor
...
#                                     outfile.write('\t%s'%(subtreeString))
            ...
            if 'subrate' in analyses:
                ...
                for g,n in nodes.items(): ## iterate over groups, nodes
...
#                    print treecount, lensubs[g]
                ...
                comboSubgroupsRate = sum([x[1] for x in lensubs.values()])/sum([x[0] for x in lensubs.values()])
                
                #Get info for full tree
                fullLen, fullSubs = subclade_branch_info(ll.root)
                #print(fullLen)
                #print(sum([x[0] for x in lensubs.values()]))
                #print(fullLen - sum([x[0] for x in lensubs.values()]))
                #print("****************************")
                #print(fullSubs)
                #print(sum([x[1] for x in lensubs.values()]))
                #print(fullSubs - sum([x[1] for x in lensubs.values()]))
                #print("****************************")
                #print("****************************")
                ...
                #Purpose of if/else statement is to catch division by zero errors.
                if (fullSubs - sum([x[1] for x in lensubs.values()]))==0 or (fullLen - sum([x[0] for x in lensubs.values()]))==0:
                    restRate=0
                else:
                    restRate = (fullSubs - sum([x[1] for x in lensubs.values()]))/(fullLen - sum([x[0] for x in lensubs.values()]))
